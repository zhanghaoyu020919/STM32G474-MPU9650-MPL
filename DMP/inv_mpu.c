...
#define log_i       printf  //¥Ú”°–≈œ¢
#define log_e       printf  //¥Ú”°–≈œ¢
...
u8 mpu_dmp_init(void)
{
	...
	//IIC_Init(); 		        //≥ı ºªØIIC◊‹œﬂ
	if(mpu_init(&int_param)==0)	//≥ı ºªØMPU9250
	{	 
        res=inv_init_mpl();     //≥ı ºªØMPL
        if(res)return 1;
        ...
        res=inv_start_mpl();    //ø™∆ÙMPL
        if(res)return 1;
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL|INV_XYZ_COMPASS);//…Ë÷√À˘–Ë“™µƒ¥´∏–∆˜
		if(res)return 2; 
		res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);   //…Ë÷√FIFO
		if(res)return 3; 
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	            //…Ë÷√≤…—˘¬ 
		if(res)return 4; 
        res=mpu_set_compass_sample_rate(1000/COMPASS_READ_MS);  //…Ë÷√¥≈¡¶º∆≤…—˘¬ 
        if(res)return 5;
		...
		res=dmp_load_motion_driver_firmware();		             //º”‘ÿdmpπÃº˛
		if(res)return 6; 
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//…Ë÷√Õ”¬›“«∑ΩœÚ
		if(res)return 7; 
		res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	            //…Ë÷√dmpπ¶ƒ‹
		    DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL);
		if(res)return 8; 
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//…Ë÷√DMP ‰≥ˆÀŸ¬ (◊Ó¥Û≤ª≥¨π˝200Hz)
		if(res)return 9;   
		res=run_self_test();		//◊‘ºÏ
		if(res)return 10;    
		res=mpu_set_dmp_state(1);	// πƒ‹DMP
		if(res)return 11;     
	}
	return 0;
}
//µ√µΩdmp¥¶¿Ì∫Ûµƒ ˝æ›(◊¢“‚,±æ∫Ø ˝–Ë“™±»Ωœ∂‡∂—’ª,æ÷≤ø±‰¡ø”–µ„∂‡)
//pitch:∏©—ˆΩ« æ´∂»:0.1°„   ∑∂Œß:-90.0°„ <---> +90.0°„
//roll:∫·πˆΩ«  æ´∂»:0.1°„   ∑∂Œß:-180.0°„<---> +180.0°„
//yaw:∫ΩœÚΩ«   æ´∂»:0.1°„   ∑∂Œß:-180.0°„<---> +180.0°„
//∑µªÿ÷µ:0,’˝≥£
//    ∆‰À˚, ß∞‹
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	**/
	...
	{
		q0 = quat[0] / q30;	//q30∏Ò Ω◊™ªªŒ™∏°µ„ ˝
		q1 = quat[1] / q30;
		q2 = quat[2] / q30;
		q3 = quat[3] / q30; 
		//º∆À„µ√µΩ∏©—ˆΩ«/∫·πˆΩ«/∫ΩœÚΩ«
...
//µ√µΩmpl¥¶¿Ì∫Ûµƒ ˝æ›(◊¢“‚,±æ∫Ø ˝–Ë“™±»Ωœ∂‡∂—’ª,æ÷≤ø±‰¡ø”–µ„∂‡)
//pitch:∏©—ˆΩ« æ´∂»:0.1°„   ∑∂Œß:-90.0°„ <---> +90.0°„
//roll:∫·πˆΩ«  æ´∂»:0.1°„   ∑∂Œß:-180.0°„<---> +180.0°„
//yaw:∫ΩœÚΩ«   æ´∂»:0.1°„   ∑∂Œß:-180.0°„<---> +180.0°„
//∑µªÿ÷µ:0,’˝≥£
//    ∆‰À˚, ß∞‹
u8 mpu_mpl_get_data(float *pitch,float *roll,float *yaw, short *sCaliFlag)
{
    ...
    if(sensors&INV_XYZ_GYRO)
    {
        ...
        inv_build_gyro(gyro,sensor_timestamp);          //∞—–¬ ˝æ›∑¢ÀÕ∏¯MPL
        mpu_get_temperature(&temperature,&sensor_timestamp);
        inv_build_temp(temperature,sensor_timestamp);   //∞—Œ¬∂»÷µ∑¢∏¯MPL£¨÷ª”–Õ”¬›“«–Ë“™Œ¬∂»÷µ
    }
    ...
    if(sensors&INV_XYZ_ACCEL)
    {
        ...
        inv_build_accel(accel,0,sensor_timestamp);      //∞—º”ÀŸ∂»÷µ∑¢∏¯MPL
    }
    ...
    if (!mpu_get_compass_reg(compass_short, &sensor_timestamp)) 
    {
        ...
        inv_build_compass(compass,0,sensor_timestamp); //∞—¥≈¡¶º∆÷µ∑¢∏¯MPL
    }
    inv_execute_on_data();
...
//    {
//    	sGyro[0] = gyro[0]; sGyro[1] = gyro[1]; sGyro[1] = gyro[1];
//        inv_build_gyro(gyro,sensor_timestamp);          //ÊääÊñ∞Êï∞ÊçÆÂèëÈÄÅÁªôMPL
//        mpu_get_temperature(&temperature,&sensor_timestamp);
//        inv_build_temp(temperature,sensor_timestamp);   //ÊääÊ∏©Â∫¶ÂÄºÂèëÁªôMPLÔºåÂè™ÊúâÈôÄËû∫‰ª™ÈúÄË¶ÅÊ∏©Â∫¶ÂÄº
//    }
//    
//    if(sensors&INV_XYZ_ACCEL)
//    {
...
//        inv_build_accel(accel,0,sensor_timestamp);      //ÊääÂä†ÈÄüÂ∫¶ÂÄºÂèëÁªôMPL
//    }
//    
//    if (!mpu_get_compass_reg(compass_short, &sensor_timestamp)) 
//    {
...
//        inv_build_compass(compass,0,sensor_timestamp); //ÊääÁ£ÅÂäõËÆ°ÂÄºÂèëÁªôMPL
//    }
//    inv_execute_on_data();
//    inv_get_sensor_type_euler(data,&accuracy,&timestamp);
//    *sCaliFlag = accuracy;
